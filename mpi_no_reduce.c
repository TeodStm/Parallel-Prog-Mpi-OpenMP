...
int main( int argc, char* argv[]){
	...
	int my_rank, comm_sz, i, j, N, M, pow_of_proc = 1, countReduce = 50, results;

	double **ArrBefore, **ArrAfter, **tmpArr;
	time_t t;
	int north, east, south, west, rowPr = 1, colPr = 1, myrow, mycol, flag = 1;
	double start_time, end_time, total_time;
	char name[MPI_MAX_PROCESSOR_NAME];
	int len, dim_size[2], periods[2], coords[2], grp_sz;
	...
	*******/
	...
	while( pow_of_proc * pow_of_proc < comm_sz ){
		pow_of_proc++;
	}
	if( pow_of_proc * pow_of_proc != comm_sz ){  //to plhthos diergasiwn den exei riza
		for(i = pow_of_proc; i < comm_sz; i++){
			if( comm_sz % i == 0 ){
				dim_size[0] = i;
	...
	else{ //uparxei h riza tou comm_sz
		dim_size[0] = pow_of_proc;
        	dim_size[1] = pow_of_proc;
	}
	...
	/** Katanomh diergasiwn ston pinaka me kartesianh topologia **/
	...
	for( i = 1; i < N+1; i++){
		for( j = 1; j < M+1; j++){
			ArrBefore[i][j] = (unsigned char) ((rand() + my_rank) % 256);
			ArrAfter[i][j] = 0.0;
		}
	}
